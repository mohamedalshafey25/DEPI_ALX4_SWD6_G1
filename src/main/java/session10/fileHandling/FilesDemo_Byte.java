...
public class FilesDemo_Byte{
    public static void main(String[] args) throws IOException {
        ...
        byte[] originalCatBytes = new byte[(int) originalCat.length()]; // 2500L = 2500 bytes > 2.5 KB (int) 2500L = 2500

        inputStream.read(originalCatBytes); // Reading the file's byte code into the byte array
        System.out.println("Image byte code read successfully from: " + originalCat.getAbsolutePath());
        inputStream.close();

        /**  print the bytes to console
         for (byte b : originalCatBytes) {
         System.out.print(b + " ");
         } */
